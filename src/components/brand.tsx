import type { ReactNode } from "react";
import { CopyButton } from "./home/interactive";

/* GitHub mark (lucide no longer ships brand icons) */
export function GithubMark({ className = "size-4" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className={className}>
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  );
}

/* Inline code accent */
export function Kbd({ children }: { children: ReactNode }) {
  return (
    <code className="rounded bg-kwiva-500/10 px-1.5 py-0.5 font-mono text-[0.85em] font-medium text-kwiva-700 dark:text-kwiva-300">
      {children}
    </code>
  );
}

/* Numbered section heading */
export function SectionHead({
  n,
  kicker,
  title,
  sub,
  align = "center",
}: {
  n: string;
  kicker: string;
  title: ReactNode;
  sub?: ReactNode;
  align?: "center" | "left";
}) {
  const centered = align === "center";
  return (
    <div className={centered ? "mx-auto mb-12 max-w-2xl" : "mb-10 max-w-2xl"}>
      <p className="mb-3 flex items-center gap-2.5 font-mono text-[11px] uppercase tracking-[0.2em] text-fd-muted-foreground">
        <span className="text-kwiva-600 dark:text-kwiva-400">{n}</span>
        <span className="h-px w-6 bg-fd-border" />
        {kicker}
      </p>
      <h2 className="font-display font-expanded text-balance text-3xl font-bold tracking-tight text-fd-foreground sm:text-4xl">
        {title}
      </h2>
      {sub ? (
        <p className="mt-4 text-pretty text-base leading-relaxed text-fd-muted-foreground">{sub}</p>
      ) : null}
    </div>
  );
}

/* Editor-style code window */
export function CodeWindow({
  title,
  children,
  copy,
}: {
  title: string;
  children: ReactNode;
  copy?: string;
}) {
  return (
    <div className="overflow-hidden rounded-lg border border-fd-border bg-fd-background">
      <div className="flex items-center justify-between gap-3 border-b border-fd-border bg-fd-secondary/40 px-4 py-2">
        <div className="flex min-w-0 items-center gap-2.5">
          <span className="size-1.5 shrink-0 rounded-full bg-kwiva-500" />
          <span className="truncate font-mono text-[11px] text-fd-muted-foreground">{title}</span>
        </div>
        {copy ? <CopyButton value={copy} /> : null}
      </div>
      <pre className="overflow-x-auto p-5 font-mono text-[12.5px] leading-[1.8]">{children}</pre>
    </div>
  );
}
