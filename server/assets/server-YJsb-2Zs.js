...
//#region node_modules/fumadocs-core/dist/server.js
...
var NOT_OPTED_IN = Symbol();
var store = new AsyncLocalStorage({ name: "fumadocs:markdown" });
/**
...
*/
var md = Object.assign(async function md(strings, ...values) {
...
async function walk(node, state) {
	...
	if ((0, import_react_react_server.isValidElement)(node)) return element(node.type, node.props, state);
...
var MAX_ATTRIBUTE_LENGTH = 1024;
var PRESENTATIONAL_PROPS = /* @__PURE__ */ new Set([
	"children",
	"className",
	"style",
	"tabIndex"
...
function attribute(key, value) {
	...
	if (typeof value === "function" || typeof value === "symbol" || (0, import_react_react_server.isValidElement)(value)) return;
	try {
		const json = JSON.stringify(value, (_, v) => (0, import_react_react_server.isValidElement)(v) ? void 0 : v);
	...
	switch (tag) {
		...
		case "ol": {
			...
			return `\n${(await Promise.all(flatten(props.children).map((child) => {
				if (!(0, import_react_react_server.isValidElement)(child)) return typeof child === "string" ? child.trim() : "";
				return walk(child, {
					...state,
					inline: false,
					list: {
...
function detectLang(props) {
	...
	for (const child of flatten(props.children)) {
		if (out) break;
		if ((0, import_react_react_server.isValidElement)(child) && child.type === "code") out = fromClass(child.props.className);
	}
	return out ?? "";
}
/**
...
function flatten(node, out = []) {
	...
	if ((0, import_react_react_server.isValidElement)(node)) {
...
//#endregion
export { renderRoute as n, renderToMarkdown as r, md as t };
