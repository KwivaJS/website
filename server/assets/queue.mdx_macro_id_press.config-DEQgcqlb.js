...
//#region content/api/queue.mdx?macro_id=press.config.tsx%23apiRef
var import_jsx_runtime_react_server = require_jsx_runtime_react_server();
var frontmatter = {
	"title": "Queue",
	"description": "@kwiva/queue — defineJob, dispatch, workers, retries, transports."
};
...
var structuredData = {
	"contents": [
		{
			"heading": void 0,
			"content": "The `@kwiva/queue` package provides `defineJob`, the factory for background jobs, and a queue transport that moves them off the request path. Jobs run on a framework-owned transport backed by Redis or the database, carry payloads validated against a declared schema, and support retries with backoff, delayed dispatch, priority, idempotency, rate limiting, and a dead-letter queue. A job can be the target of a scheduled task, the handler behind a queued event listener, or a unit of work dispatched straight from a controller."
		},
		{
			"heading": "the-definejob-factory",
			"content": "A job file lives in `src/app/jobs/` and is auto-discovered by name."
		},
		{
			"heading": "the-definejob-factory",
			"content": "Argument"
		...
		},
		{
			...
			"content": "The handler receives a single context object, destructured in the signature above."
		},
		{
			"heading": "handler-context",
			"content": "Property"
		...
		{
			"heading": "handler-context",
			"content": "Runtime handle for the running attempt"
		},
		{
			"heading": "handler-context",
			"content": "`logger`"
		...
		},
		{
			...
			"content": "`job.progress(n)` records a 0–100 completion value against the running job. The handler's return value is captured as the job result for tracing and observability."
		},
		{
			"heading": "job-options",
			"content": "Option"
		...
		{
			"heading": "job-options",
			"content": "Queue to run on; must be declared in the queue config. Defaults to `'default'`"
		},
		{
			"heading": "job-options",
			"content": "`schema`"
		...
		{
			"heading": "job-options",
			"content": "Validation shape for `payload`"
		},
		{
			"heading": "job-options",
			"content": "`attempts`"
		...
		{
			"heading": "job-options",
			"content": "Attempts before the job is failed"
		},
		{
			"heading": "job-options",
			"content": "`backoff`"
		...
		{
			"heading": "job-options",
			"content": "`'fixed' \\| 'exponential' \\| (attempt) => number`"
		},
		{
			"heading": "job-options",
			"content": "Delay between attempts"
		},
		{
			"heading": "job-options",
			"content": "`priority`"
		...
		{
			"heading": "job-options",
			"content": "Priority value assigned at dispatch"
		},
		{
			"heading": "job-options",
			"content": "`rateLimit`"
		...
		{
			"heading": "job-options",
			"content": "`{ max: number; per: number }`"
		},
		{
			"heading": "job-options",
			"content": "Per-job rate limit (v1.x)"
		},
		{
			"heading": "job-options",
		...
		{
			"heading": "job-options",
			"content": "Deduplicates identical work"
		},
		{
			"heading": "payload-validation-and-typing",
			"content": "The `schema` option validates every payload at dispatch time and types it inside the handler. Shorthand tokens describe primitive shapes, and where a payload references a model identity the type is derived from the model IR so `payload.userId` is fully typed across dispatch and handler."
		},
		{
			"heading": "retries-and-backoff",
			"content": "`attempts` bounds how many times the job runs before it is moved to the dead-letter queue. `backoff` controls the wait between attempts."
		},
		{
			"heading": "retries-and-backoff",
			"content": "Value"
		...
		{
			"heading": "retries-and-backoff",
			"content": "Constant delay between attempts"
		},
		{
			"heading": "retries-and-backoff",
			"content": "`'exponential'`"
		...
		{
			"heading": "retries-and-backoff",
			"content": "Delay grows exponentially per attempt"
		},
		{
			"heading": "retries-and-backoff",
		...
		{
			"heading": "retries-and-backoff",
			"content": "Custom function returning the seconds to wait"
		},
		{
			"heading": "idempotency-keys",
			"content": "When a key is supplied, dispatching the same logical work twice is deduplicated: a second dispatch carrying an already-seen key is dropped. Deriving the key from the payload (as above) is the reliable pattern."
		},
		{
			"heading": "priority-and-rate-limiting",
			"content": "`priority` assigns the job a priority weight in the queue. `rateLimit: { max, per }` caps how often a job can run within a window (v1.x); queues can carry their own rate limit as well (v1.x)."
		},
		{
			"heading": "dispatch",
			"content": "Dispatch is available both as a static method on the job and through the `queue` object."
		},
		{
			"heading": "dispatch-options",
			"content": "Option"
		},
		{
			"heading": "dispatch-options",
		...
		{
			"heading": "dispatch-options",
			"content": "`number \\| DateTime`"
		},
		{
			"heading": "dispatch-options",
			"content": "Run the job later: seconds, or an absolute time"
		},
		{
			"heading": "dispatch-options",
			"content": "`queue`"
		...
		{
			"heading": "dispatch-options",
			"content": "Override the queue for this dispatch"
		},
		{
			"heading": "chaining",
			"content": "Chains run jobs in sequence, one after another (v1.x):"
		},
		{
			"heading": "batching",
			"content": "Batches dispatch a set of jobs together and report per-job outcomes (v1.x):"
		},
		{
			"heading": "workers",
			"content": "Workers consume queues and run the jobs. Run a worker in a separate process with `kwiva queue:work`, scoped by `--queue` and scaled with `--concurrency`; use `kwiva queue:listen` in development for verbose, always-on consumption."
		},
		{
			"heading": "workers",
			"content": "Flag"
		...
		{
			"heading": "workers",
			"content": "Workers deploy as their own process via `kwiva deploy --entry worker` using the standard deploy-anywhere presets, or run in-process during development."
		},
		{
			"heading": "dead-letter-queue",
			"content": "A job that exhausts `attempts` is not retried again. It is moved to the dead-letter queue — the `failed_jobs` table — where it can be inspected and requeued."
		},
		{
			"heading": "configuration-and-transports",
			"content": "Queues and drivers are declared in `src/config/queue.ts` with `defineConfig`."
		},
		{
			"heading": "configuration-and-transports",
			"content": "Option"
		...
		{
			"heading": "transport-drivers",
			"content": "Zero-infrastructure"
		},
		{
			"heading": "transport-drivers",
			"content": "`kwiva_jobs` table polling, suited to file-backed databases"
		},
		{
			"heading": "transport-drivers",
			"content": "`memory`"
		...
		{
			"heading": "transport-drivers",
			"content": "Development and tests"
		},
		{
			"heading": "transport-drivers",
			"content": "Synchronous and instant; pairs with `queue.fake()` for assertions"
		},
		{
		...
		},
		{
			...
			"content": "Queued event listeners run through the queue transport by default. A listener marked synchronous opts out and executes inline; see Events for listener configuration."
		},
		{
			"heading": "testing",
			"content": "Swap the transport for an in-memory fake and assert what was pushed:"
		},
		{
			"heading": "observability",
			"content": "Each job produces an OTel span covering dispatch, start, and finish, including the time spent waiting in the queue. Metrics are collected for queue depth, throughput, failure rate, and dead-letter queue size. Run workers with `--trace` to correlate logs per job id."
		},
		{
			"heading": "what-to-read-next",
			"content": "Jobs — Defining jobs guide"
		},
		{
			"heading": "what-to-read-next",
			"content": "Queues & Workers — Worker commands and transports"
		},
		{
			"heading": "what-to-read-next",
			"content": "Scheduling — Cron-driven tasks that dispatch jobs"
		},
		{
			"heading": "what-to-read-next",
			"content": "Queue Observability — Spans, metrics, and logs"
		},
		{
			"heading": "what-to-read-next",
			"content": "Events & Broadcasting — Queued event listeners"
		},
		{
			"heading": "what-to-read-next",
			"content": "Auth Hooks — Dispatching jobs from `onSignUp` and friends"
		}
	],
	"headings": [
		{
			"id": "the-definejob-factory",
			"content": "The `defineJob` Factory"
		},
		{
			"id": "handler-context",
		...
		{
			"id": "payload-validation-and-typing",
			"content": "Payload validation and typing"
		},
		{
			"id": "retries-and-backoff",
			"content": "Retries and backoff"
		},
		{
			"id": "idempotency-keys",
		...
		{
			"id": "priority-and-rate-limiting",
			"content": "Priority and rate limiting"
		},
		{
			"id": "dispatch",
			"content": "Dispatch"
		...
		{
			"id": "configuration-and-transports",
			"content": "Configuration and Transports"
		},
		{
...
var toc = [
	...
	},
	{
		...
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Reporting progress" })
	},
	{
		depth: 2,
		url: "#job-options",
	...
	{
		depth: 3,
		url: "#payload-validation-and-typing",
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Payload validation and typing" })
	},
	{
		depth: 3,
		url: "#retries-and-backoff",
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Retries and backoff" })
	},
	{
		depth: 3,
	...
	{
		depth: 3,
		url: "#priority-and-rate-limiting",
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Priority and rate limiting" })
	},
	{
		depth: 2,
		url: "#dispatch",
	...
	},
	{
		...
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Dispatch options" })
	},
	{
		depth: 3,
		url: "#chaining",
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Chaining" })
	},
	{
		depth: 3,
		url: "#batching",
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Batching" })
	},
	{
		depth: 2,
		url: "#workers",
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Workers" })
	},
	{
		depth: 2,
		url: "#dead-letter-queue",
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Dead-Letter Queue" })
	},
	{
		depth: 2,
		url: "#configuration-and-transports",
		title: (0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: "Configuration and Transports" })
	},
	{
		depth: 3,
...
function _createMdxContent(props) {
	...
	return (0, import_jsx_runtime_react_server.jsxs)(import_jsx_runtime_react_server.Fragment, { children: [
		...
		(0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: (0, import_jsx_runtime_react_server.jsx)(_components.pre, {
			...
			tabIndex: "0",
			title: "the-definejob-factory.ts",
		...
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			...
			" and is auto-discovered by name."
		] }),
		"\n",
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.p, { children: "The handler receives a single context object, destructured in the signature above." }),
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.table, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.thead, { children: (0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
			...
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Runtime handle for the running attempt" })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "job.progress(n)" }), " records a 0–100 completion value against the running job. The handler's return value is captured as the job result for tracing and observability."] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.h2, {
			id: "job-options",
		...
		(0, import_jsx_runtime_react_server.jsxs)(_components.table, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.thead, { children: (0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
			...
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "string" }) }),
				(0, import_jsx_runtime_react_server.jsxs)(_components.td, { children: ["Queue to run on; must be declared in the queue config. Defaults to ", (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "'default'" })] })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "PayloadSchema" }) }),
				(0, import_jsx_runtime_react_server.jsxs)(_components.td, { children: ["Validation shape for ", (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "payload" })] })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Attempts before the job is failed" })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "'fixed' | 'exponential' | (attempt) => number" }) }),
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Delay between attempts" })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "number" }) }),
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Priority value assigned at dispatch" })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "{ max: number; per: number }" }) }),
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Per-job rate limit (v1.x)" })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Deduplicates identical work" })
			] })
		] })] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.h3, {
			id: "payload-validation-and-typing",
			children: "Payload validation and typing"
		}),
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			"The ",
			(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "schema" }),
			" option validates every payload at dispatch time and types it inside the handler. Shorthand tokens describe primitive shapes, and where a payload references a model identity the type is derived from the model IR so ",
			(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "payload.userId" }),
			" is fully typed across dispatch and handler."
		] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: (0, import_jsx_runtime_react_server.jsx)(_components.pre, {
			className: "shiki shiki-themes github-light github-dark",
			...
			tabIndex: "0",
			title: "payload-validation-and-typing.ts",
			icon: "<svg viewBox=\"0 0 24 24\"><path d=\"M1.125 0C.502 0 0 .502 0 1.125v21.75C0 23.498.502 24 1.125 24h21.75c.623 0 1.125-.502 1.125-1.125V1.125C24 .502 23.498 0 22.875 0zm17.363 9.75c.612 0 1.154.037 1.627.111a6.38 6.38 0 0 1 1.306.34v2.458a3.95 3.95 0 0 0-.643-.361 5.093 5.093 0 0 0-.717-.26 5.453 5.453 0 0 0-1.426-.2c-.3 0-.573.028-.819.086a2.1 2.1 0 0 0-.623.242c-.17.104-.3.229-.393.374a.888.888 0 0 0-.14.49c0 .196.053.373.156.529.104.156.252.304.443.444s.423.276.696.41c.273.135.582.274.926.416.47.197.892.407 1.266.628.374.222.695.473.963.753.268.279.472.598.614.957.142.359.214.776.214 1.253 0 .657-.125 1.21-.373 1.656a3.033 3.033 0 0 1-1.012 1.085 4.38 4.38 0 0 1-1.487.596c-.566.12-1.163.18-1.79.18a9.916 9.916 0 0 1-1.84-.164 5.544 5.544 0 0 1-1.512-.493v-2.63a5.033 5.033 0 0 0 3.237 1.2c.333 0 .624-.03.872-.09.249-.06.456-.144.623-.25.166-.108.29-.234.373-.38a1.023 1.023 0 0 0-.074-1.089 2.12 2.12 0 0 0-.537-.5 5.597 5.597 0 0 0-.807-.444 27.72 27.72 0 0 0-1.007-.436c-.918-.383-1.602-.852-2.053-1.405-.45-.553-.676-1.222-.676-2.005 0-.614.123-1.141.369-1.582.246-.441.58-.804 1.004-1.089a4.494 4.494 0 0 1 1.47-.629 7.536 7.536 0 0 1 1.77-.201zm-15.113.188h9.563v2.166H9.506v9.646H6.789v-9.646H3.375z\" fill=\"currentColor\" /></svg>",
			children: (0, import_jsx_runtime_react_server.jsxs)(_components.code, { children: [
				(0, import_jsx_runtime_react_server.jsxs)(_components.span, {
					className: "line",
				...
				(0, import_jsx_runtime_react_server.jsxs)(_components.span, {
					...
					children: [
						...
						(0, import_jsx_runtime_react_server.jsx)(_components.span, {
							...
							children: "({ period: payload.period })"
						})
					]
				}),
				"\n",
		...
		(0, import_jsx_runtime_react_server.jsxs)(_components.table, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.thead, { children: (0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.th, { children: "Value" }), (0, import_jsx_runtime_react_server.jsx)(_components.th, { children: "Behavior" })] }) }), (0, import_jsx_runtime_react_server.jsxs)(_components.tbody, { children: [
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "'fixed'" }) }), (0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Constant delay between attempts" })] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "'exponential'" }) }), (0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Delay grows exponentially per attempt" })] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "(attempt) => number" }) }), (0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Custom function returning the seconds to wait" })] })
		] })] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.h3, {
			id: "idempotency-keys",
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.p, { children: "When a key is supplied, dispatching the same logical work twice is deduplicated: a second dispatch carrying an already-seen key is dropped. Deriving the key from the payload (as above) is the reliable pattern." }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.h3, {
			id: "priority-and-rate-limiting",
			children: "Priority and rate limiting"
		}),
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "priority" }),
			" assigns the job a priority weight in the queue. ",
			(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "rateLimit: { max, per }" }),
			" caps how often a job can run within a window (v1.x); queues can carry their own rate limit as well (v1.x)."
		] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.h2, {
			id: "dispatch",
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			"Dispatch is available both as a static method on the job and through the ",
		...
		(0, import_jsx_runtime_react_server.jsxs)(_components.table, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.thead, { children: (0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
			...
			(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "number | DateTime" }) }),
			(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Run the job later: seconds, or an absolute time" })
		] }), (0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
			(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "queue" }) }),
			(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: (0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "string" }) }),
			(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Override the queue for this dispatch" })
		] })] })] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.h3, {
			id: "chaining",
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.p, { children: "Chains run jobs in sequence, one after another (v1.x):" }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: (0, import_jsx_runtime_react_server.jsx)(_components.pre, {
			className: "shiki shiki-themes github-light github-dark",
			style: {
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.p, { children: "Batches dispatch a set of jobs together and report per-job outcomes (v1.x):" }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: (0, import_jsx_runtime_react_server.jsx)(_components.pre, {
			className: "shiki shiki-themes github-light github-dark",
			style: {
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			"Workers consume queues and run the jobs. Run a worker in a separate process with ",
			...
			" in development for verbose, always-on consumption."
		] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: (0, import_jsx_runtime_react_server.jsx)(_components.pre, {
			className: "shiki shiki-themes github-light github-dark",
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			"Workers deploy as their own process via ",
			(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "kwiva deploy --entry worker" }),
			" using the standard deploy-anywhere presets, or run in-process during development."
		] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.h2, {
			id: "dead-letter-queue",
		...
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			"A job that exhausts ",
			(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "attempts" }),
			" is not retried again. It is moved to the dead-letter queue — the ",
			(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "failed_jobs" }),
			" table — where it can be inspected and requeued."
		] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: (0, import_jsx_runtime_react_server.jsx)(_components.pre, {
			className: "shiki shiki-themes github-light github-dark",
				...
				(0, import_jsx_runtime_react_server.jsxs)(_components.span, {
					...
					children: [
						...
						(0, import_jsx_runtime_react_server.jsx)(_components.span, {
							...
							children: "      # requeue a single failed job"
						})
					]
				}),
				"\n",
				(0, import_jsx_runtime_react_server.jsxs)(_components.span, {
					...
					children: [
						...
						(0, import_jsx_runtime_react_server.jsx)(_components.span, {
							...
							children: "     # requeue every failed job"
						})
					]
				})
			] })
		...
		(0, import_jsx_runtime_react_server.jsx)(_components.h2, {
			id: "configuration-and-transports",
			children: "Configuration and Transports"
		}),
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			"Queues and drivers are declared in ",
		...
		(0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: (0, import_jsx_runtime_react_server.jsx)(_components.pre, {
			...
			tabIndex: "0",
			title: "configuration-and-transports.ts",
			icon: "<svg viewBox=\"0 0 24 24\"><path d=\"M1.125 0C.502 0 0 .502 0 1.125v21.75C0 23.498.502 24 1.125 24h21.75c.623 0 1.125-.502 1.125-1.125V1.125C24 .502 23.498 0 22.875 0zm17.363 9.75c.612 0 1.154.037 1.627.111a6.38 6.38 0 0 1 1.306.34v2.458a3.95 3.95 0 0 0-.643-.361 5.093 5.093 0 0 0-.717-.26 5.453 5.453 0 0 0-1.426-.2c-.3 0-.573.028-.819.086a2.1 2.1 0 0 0-.623.242c-.17.104-.3.229-.393.374a.888.888 0 0 0-.14.49c0 .196.053.373.156.529.104.156.252.304.443.444s.423.276.696.41c.273.135.582.274.926.416.47.197.892.407 1.266.628.374.222.695.473.963.753.268.279.472.598.614.957.142.359.214.776.214 1.253 0 .657-.125 1.21-.373 1.656a3.033 3.033 0 0 1-1.012 1.085 4.38 4.38 0 0 1-1.487.596c-.566.12-1.163.18-1.79.18a9.916 9.916 0 0 1-1.84-.164 5.544 5.544 0 0 1-1.512-.493v-2.63a5.033 5.033 0 0 0 3.237 1.2c.333 0 .624-.03.872-.09.249-.06.456-.144.623-.25.166-.108.29-.234.373-.38a1.023 1.023 0 0 0-.074-1.089 2.12 2.12 0 0 0-.537-.5 5.597 5.597 0 0 0-.807-.444 27.72 27.72 0 0 0-1.007-.436c-.918-.383-1.602-.852-2.053-1.405-.45-.553-.676-1.222-.676-2.005 0-.614.123-1.141.369-1.582.246-.441.58-.804 1.004-1.089a4.494 4.494 0 0 1 1.47-.629 7.536 7.536 0 0 1 1.77-.201zm-15.113.188h9.563v2.166H9.506v9.646H6.789v-9.646H3.375z\" fill=\"currentColor\" /></svg>",
			children: (0, import_jsx_runtime_react_server.jsxs)(_components.code, { children: [
				(0, import_jsx_runtime_react_server.jsxs)(_components.span, {
					className: "line",
		...
		(0, import_jsx_runtime_react_server.jsxs)(_components.table, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.thead, { children: (0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
			...
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Redis transport" })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Zero-infrastructure" }),
				(0, import_jsx_runtime_react_server.jsxs)(_components.td, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "kwiva_jobs" }), " table polling, suited to file-backed databases"] })
			] }),
			(0, import_jsx_runtime_react_server.jsxs)(_components.tr, { children: [
				...
				(0, import_jsx_runtime_react_server.jsx)(_components.td, { children: "Development and tests" }),
				(0, import_jsx_runtime_react_server.jsxs)(_components.td, { children: [
					"Synchronous and instant; pairs with ",
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			"Queued event listeners run through the queue transport by default. A listener marked synchronous opts out and executes inline; see ",
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.p, { children: "Swap the transport for an in-memory fake and assert what was pushed:" }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(import_jsx_runtime_react_server.Fragment, { children: (0, import_jsx_runtime_react_server.jsx)(_components.pre, {
			className: "shiki shiki-themes github-light github-dark",
			style: {
		...
		"\n",
		(0, import_jsx_runtime_react_server.jsxs)(_components.p, { children: [
			"Each job produces an OTel span covering dispatch, start, and finish, including the time spent waiting in the queue. Metrics are collected for queue depth, throughput, failure rate, and dead-letter queue size. Run workers with ",
			(0, import_jsx_runtime_react_server.jsx)(_components.code, { children: "--trace" }),
			" to correlate logs per job id."
		] }),
		"\n",
		(0, import_jsx_runtime_react_server.jsx)(_components.h2, {
			id: "what-to-read-next",
		...
		(0, import_jsx_runtime_react_server.jsxs)(_components.ul, { children: [
			...
			}), " — Worker commands and transports"] }),
			"\n",
			(0, import_jsx_runtime_react_server.jsxs)(_components.li, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.a, {
				href: "/docs/background-work/scheduling",
				children: "Scheduling"
			}), " — Cron-driven tasks that dispatch jobs"] }),
			"\n",
			(0, import_jsx_runtime_react_server.jsxs)(_components.li, { children: [(0, import_jsx_runtime_react_server.jsx)(_components.a, {
				href: "/docs/background-work/observability",
				children: "Queue Observability"
